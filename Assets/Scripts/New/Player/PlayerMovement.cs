...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void MoveToPosition(float newPosition)
    {
        ...
        if (!isMoving)
        {
            StartCoroutine(OnDodge(newPosition));
        }
    }
    ...
    private IEnumerator OnDodge(float xPosition)
    {
        isMoving = true;
        ...
        while (Mathf.Abs(playerTransform.position.x - xPosition) > 0.01f)
        {
            ...
            // Ensuite, j'introduis nous nouvelle variable qui va conserver la position du joueur.
            ...
            position = Vector3.MoveTowards(position,
                new Vector3(xPosition, playerPosition.y, playerPosition.z), dodgeSpeed * Time.deltaTime);
